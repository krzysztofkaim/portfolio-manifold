export const siteMetadata = {
  defaultLocale: 'en',
  keywords: [
    'krzysztof kaim',
    'full-stack engineer',
    'react engineer',
    'typescript engineer',
    '.net engineer',
    'software architect',
    'systems architecture',
    'enterprise software',
    'product engineering',
    'delivery engineering',
    'frontend engineer',
    'backend engineer',
    'react',
    'typescript',
    '.net',
    'software engineer portfolio',
    'interactive portfolio',
    'astro portfolio'
  ],
  name: 'Krzysztof Kaim',
  profession:
    'Software Engineer, Lead Engineer and Solution Architect specializing in React, .NET, MES, ERP integrations and industrial systems',
  sameAs: [
    'https://github.com/krzysztofkaim',
    'https://www.linkedin.com/in/krzysztofkaim/'
  ],
  socialImagePath: '/social-card.png',
  titleTemplate: '%s | Krzysztof Kaim',
  websiteName: 'Krzysztof Kaim'
} as const;

export const homePageSeoCopy = {
  en: {
    headline:
      'Krzysztof Kaim | Software Engineer for React, .NET and Industrial Systems',
    intro:
      'Krzysztof Kaim is a software engineer specializing in React, TypeScript, .NET, MES, ERP integrations, industrial systems, and distressed enterprise project recovery.',
    sections: [
      'Profile covering engineering focus, delivery style and technical range.',
      'Deployments covering production work in enterprise, consulting and education contexts.',
      'Operations covering system design, architecture, integrations and execution.',
      'Capabilities covering frontend, backend, infrastructure and delivery skills.',
      'Credentials covering education, communication and compliance context.'
    ]
  },
  pl: {
    headline:
      'Krzysztof Kaim | Software Engineer dla React, .NET i systemów przemysłowych',
    intro:
      'Krzysztof Kaim to software engineer specjalizujący się w React, TypeScript, .NET, systemach MES, integracjach ERP, systemach przemysłowych i ratowaniu trudnych projektów enterprise.',
    sections: [
      'Profil: fokus inżynierski, styl dowożenia i zakres techniczny.',
      'Wdrożenia: praca produkcyjna w środowiskach enterprise, consultingu i edukacji.',
      'Operacje: projektowanie systemów, architektura, integracje i egzekucja.',
      'Umiejętności: kompetencje frontendowe, backendowe, infrastrukturalne i delivery.',
      'Kwalifikacje: edukacja, komunikacja i kontekst zgodności.'
    ]
  }
} as const;

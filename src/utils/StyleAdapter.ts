...
const cssTypedFactory = (
  window as Window & { CSS?: Partial<CssTypedNumericFactory> }
).CSS;
const HAS_TYPED_OM =
...
export class StyleAdapter {
  ...
  static setNumericProperty(
    ...
    if (HAS_TYPED_OM) {
      try {
        (el as TypedStyleMapCapableElement).attributeStyleMap.set(
          name,
          this.createTypedValue(value, unit)
        );
        return;
      } catch {
        // Fallback
   ...
   */
  static setNumericProperties(
    el: HTMLElement,
    properties: Record<string, { value: number; unit?: TypedCssUnit }>
  ): void {
    if (HAS_TYPED_OM) {
      ...
      for (const [name, { value, unit }] of Object.entries(properties)) {
        try {
          ...
          /* Fallback handled by the loop if needed, but typed OM is usually robust here */
        }
      }
      return;
    }
  ...
  private static createTypedValue(value: number, unit?: TypedCssUnit): unknown {
    if (!cssTypedFactory || typeof cssTypedFactory.number !== 'function')
      return value;
    if (!unit) return cssTypedFactory.number(value);
    if (unit === 'px' && typeof cssTypedFactory.px === 'function')
      return cssTypedFactory.px(value);
    if (unit === 'deg' && typeof cssTypedFactory.deg === 'function')
      return cssTypedFactory.deg(value);
    if (unit === 'percent' && typeof cssTypedFactory.percent === 'function')
      return cssTypedFactory.percent(value);

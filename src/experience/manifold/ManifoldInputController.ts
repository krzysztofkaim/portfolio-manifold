...
export class ManifoldInputController {
  ...
  attach(): void {
    this.runtime.addWindowEventListener(
      'pointermove',
      this.handlers.handlePointerMove,
      { passive: true }
    );
    this.runtime.addWindowEventListener(
      'pointerleave',
      this.handlers.handlePointerLeave
    );
    this.runtime.addWindowEventListener(
      'keydown',
      this.handlers.handleGlobalKeydown
    );
    this.runtime.addWindowEventListener('wheel', this.handlers.handleWheel, {
      passive: true
    });
    this.bindings.viewport.addEventListener(
      'pointerdown',
      this.handlers.handleViewportPointerDown,
      { passive: true }
    );
    this.bindings.viewport.addEventListener(
      'pointerup',
      this.handlers.handleViewportPointerUp,
      { passive: true }
    );
    this.bindings.viewport.addEventListener(
      'click',
      this.handlers.handleViewportClick
    );
    this.bindings.featuredInteractiveEl?.addEventListener(
      'click',
      this.handlers.handleFeaturedClick
    );
    this.bindings.featuredInteractiveEl?.addEventListener(
      'keydown',
      this.handlers.handleFeaturedKeydown
    );
    this.bindings.exitButton.addEventListener(
      'click',
      this.handlers.handleExitClick
    );
  }
  ...
  detach(): void {
    ...
    this.bindings.viewport.removeEventListener(
      'pointerdown',
      this.handlers.handleViewportPointerDown
    );
    this.bindings.viewport.removeEventListener(
    ...
    this.bindings.featuredInteractiveEl?.removeEventListener(
      'click',
      this.handlers.handleFeaturedClick
    );
    this.bindings.featuredInteractiveEl?.removeEventListener(
    ...
    this.bindings.exitButton.removeEventListener(
      'click',
      this.handlers.handleExitClick
    );
  }

...
export class ManifoldHintController {
  ...
  updateIntroHint(): void {
    ...
    const targetLabelY =
      ...
      copyHeight -
      clamp(
        rect.height * (isMobile ? 0.08 : 0.09),
        isMobile ? 18 : 24,
        isMobile ? 28 : 34
      );
    ...
    const anchorX = lerp(
      introState.anchorX || targetAnchorX,
      targetAnchorX,
      0.18
    );
    const anchorY = lerp(
      introState.anchorY || targetAnchorY,
      targetAnchorY,
      0.18
    );
    ...
    if (geometry !== introState.geometry) {
      introState.hint.style.setProperty(
        '--intro-copy-x',
        `${labelX.toFixed(2)}px`
      );
      introState.hint.style.setProperty(
  ...
  updateContextHint(time: number): void {
    ...
    const hintMotionKey =
      itemX * 10000000 + itemY * 10000 + sceneIdx * 1000 + viewportKey;
    ...
    const labelGap = clamp(
      Math.min(rect.width, copyWidth) * 0.024,
      viewport.isMobile ? 8 : 10,
      viewport.isMobile ? 14 : 18
    );
    ...
    const fitsRight =
      preferredRightLabelX + copyWidth <= viewport.width - viewportInset;
    const fitsLeft = preferredLeftLabelX >= viewportInset;
    const hintSide: 'left' | 'right' =
      !fitsRight && fitsLeft ? 'left' : 'right';
    const targetAnchorX =
      hintSide === 'right' ? rect.right - anchorInset : rect.left + anchorInset;
    const unclampedLabelY =
      rect.top +
      rect.height * (viewport.isMobile ? 0.24 : 0.21) -
      copyHeight * 0.5;
    const targetAnchorY =
      rect.top + rect.height * (viewport.isMobile ? 0.54 : 0.5);
    ...
    const joinX =
      hintSide === 'right'
        ? targetLabelX - elbowOffset
        : targetLabelX + copyWidth + elbowOffset;
    ...
    if (geometry !== contextState.geometry) {
      contextState.hint.style.setProperty(
        '--context-copy-x',
        `${labelX.toFixed(2)}px`
      );
      contextState.hint.style.setProperty(

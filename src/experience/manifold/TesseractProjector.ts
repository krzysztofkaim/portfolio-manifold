...
export const TESSERACT_VERTICES: readonly HyperVertex[] =
  createTesseractVertices();
export const TESSERACT_EDGES: readonly (readonly [number, number])[] =
  createTesseractEdges(TESSERACT_VERTICES);
export const TESSERACT_FACES: readonly TesseractFaceDefinition[] =
  createTesseractFaces(TESSERACT_VERTICES);
...
export class TesseractProjector {
  ...
  static computeScene(input: TesseractProjectionInput): FourDSceneState {
    ...
    const xwAngle = spin;
    const yzAngle =
      spin * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.yzSpinScalar +
      input.time * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.yzTimeScalar;
    const zwAngle =
      spin * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.zwSpinScalar +
      input.time * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.zwTimeScalar;
    const xyAngle =
      spin * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.xySpinScalar +
      input.time * MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.xyTimeScalar;
    ...
    const scale =
      ...
      MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.screenScaleFactor;
    ...
    if (
      sceneKey === TesseractProjector._lastSceneKey &&
      TesseractProjector._lastScene
    ) {
    ...
    const rawFaceStates: TesseractFaceProjection[] = TESSERACT_FACES.map(
      (face) => {
        const points = face.verts.map(
        ...
        const localPointA: readonly [number, number] = [
          pointA.x - worldOriginX,
          pointA.y - worldOriginY
        ];
        const localPointB: readonly [number, number] = [
          pointB.x - worldOriginX,
          pointB.y - worldOriginY
        ];
        const localPointC: readonly [number, number] = [
          pointC.x - worldOriginX,
          pointC.y - worldOriginY
        ];
        const localPointD: readonly [number, number] = [
          pointD.x - worldOriginX,
          pointD.y - worldOriginY
        ];
        const cross =
          (localPointB[0] - localPointA[0]) *
            (localPointD[1] - localPointA[1]) -
          (localPointB[1] - localPointA[1]) * (localPointD[0] - localPointA[0]);
        const diagonal = Math.hypot(
          localPointC[0] - localPointA[0],
          localPointC[1] - localPointA[1]
        );
        const avgZ = (pointA.z3 + pointB.z3 + pointC.z3 + pointD.z3) * 0.25;
        const avgW = (pointA.w4 + pointB.w4 + pointC.w4 + pointD.w4) * 0.25;
        const centerFaceX =
          (localPointA[0] + localPointB[0] + localPointC[0] + localPointD[0]) *
          0.25;
        const centerFaceY =
          (localPointA[1] + localPointB[1] + localPointC[1] + localPointD[1]) *
          0.25;
        const depthWeight = clamp(
          (avgZ - MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.depthNear) /
            MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.depthSpan,
          0,
          1
        );
        const edgeWeight = clamp(1 - Math.abs(avgW) * 0.26, 0.48, 1);
        const alpha = clamp(
          (0.12 + depthWeight * 0.88) * edgeWeight * input.fourDProgress,
          0.06,
          0.98
        );
        const matrix =
          Math.abs(cross) > 0.01 &&
          diagonal >= MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.diagonalCullFloor
    ...
    for (let order = 0; order < sortedByDepth.length; order += 1) {
      rawFaceStates[sortedByDepth[order]!.index]!.zIndex =
        order + MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.faceZIndexBase;
    }

    const edgeStates: TesseractEdgeProjection[] = TESSERACT_EDGES.map(
      (edge) => {
        ...
        return {
          ...
          pointB,
          wEdge:
            TESSERACT_VERTICES[edge[0]]![3] !== TESSERACT_VERTICES[edge[1]]![3],
...
export function projectHyperVertex(
  ...
  const wLens =
    1 /
    Math.max(
      MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.wLensFloor,
      MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.wLensOrigin - w
    );
  const x3 = x * wLens;
  const y3 = y * wLens;
  const z3 = z * wLens;
  const zLens =
    1 /
    Math.max(
      MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.zLensFloor,
      MANIFOLD_CONSTANTS.TESSERACT_PHYSICS.zLensOrigin - z3
    );
...
function createTesseractFaces(
  vertices: readonly HyperVertex[]
): readonly TesseractFaceDefinition[] {
      ...
      for (const fixedValueA of [-1, 1] as const) {
        for (const fixedValueB of [-1, 1] as const) {
          ...
          for (
            ...
            if (
              vertex[fixedAxes[0]] === fixedValueA &&
              vertex[fixedAxes[1]] === fixedValueB
            ) {
          ...
          const centerA =
            quad.reduce(
              (sum, vertexIndex) => sum + vertices[vertexIndex]![axisA],
              0
            ) / 4;
          const centerB =
            quad.reduce(
              (sum, vertexIndex) => sum + vertices[vertexIndex]![axisB],
              0
            ) / 4;
          quad.sort((left, right) => {
            ...
            const leftAngle = Math.atan2(
              leftVertex[axisB] - centerB,
              leftVertex[axisA] - centerA
            );
            const rightAngle = Math.atan2(
              rightVertex[axisB] - centerB,
              rightVertex[axisA] - centerA
            );

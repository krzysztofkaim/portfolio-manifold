...
export class ManifoldVisualStateManager {
  ...
  update(input: VisualStateInput): void {
    ...
    const twoDFastEligible =
      (is2DMode ||
        viewModeProgress >
          MANIFOLD_CONSTANTS.PERFORMANCE_THRESHOLDS.fastModeViewThreshold ||
        viewModeTarget >
          MANIFOLD_CONSTANTS.PERFORMANCE_THRESHOLDS.fastModeViewThreshold) &&
      !expandedCard;

    const velocityPressure = clamp(
      (velocityMagnitude - fastVelocityFloor * 0.18) /
        Math.max(0.001, fastVelocityCeiling - fastVelocityFloor * 0.18),
      0,
      1
    );
    ...
    const framePressure = clamp(
      (Math.max(frameTimeEma, frameTimeBurst) - highRefreshFrameBudget) /
        frameStressSpan,
      0,
      1
    );
    ...
    const rawTwoDVisualMix = clamp(
      viewModeProgress * (1 - clamp(fourDTransitionProgress * 1.15, 0, 1)),
      0,
      1
    );
    ...
    const frameStressState =
      ...
      frameTimeBurst >
        highRefreshFrameBudget *
          MANIFOLD_CONSTANTS.PERFORMANCE_THRESHOLDS.frameStressBurstMultiplier;
    ...
    const isVelocityActive = this.lastIsScrollingState
  ...
  syncModeStates(input: {
    ...
    const twoDPresentationProgress = introCompleted
      ? clamp(
          exitingFourDTo2D
            ? Math.max(
                viewModeProgress,
                (1 - clamp(fourDTransitionProgress, 0, 1)) * 1.24 + 0.04
              )
            : viewModeProgress,
          0,
          1
    ...
    const cardMorphingState =
      expandedCard !== null &&
      Math.abs(expandedTarget - expandedProgress) > 0.015;
    ...
    const visualFourDModeState =
      ...
      (targetViewMode === '4d' ||
        ...
        (this.lastVisualFourDModeState
          ? fourDTransitionProgress > 0.04
          : fourDTransitionProgress > 0.18));
    ...
    const visualExpandedState =
      expandedCard !== null &&
      (this.lastVisualExpandedState
        ? expandedProgress > 0.08
        : expandedProgress > 0.2);
  ...
  private updateDynamicCSSVars(
    cardFastness: number,
    twoDVisualMix: number
  ): void {
    const precision = IS_SAFARI ? 2 : 3;
    const cardVisualState =
      twoDVisualMix.toFixed(precision) + '|' + cardFastness.toFixed(precision);
    ...
    if (cardVisualState !== this.lastTwoDCardVisualState) {
      if (!this.dynamicVarsEl) {
        ...
        this.dynamicVarsRule = (
          sheet.cssRules[ruleIndex] as CSSStyleRule
        ).style;
      }
      ...
      if (
        this.dynamicVarsRule &&
        (!IS_SAFARI || cardFastnessDelta > 0.08 || visualMixDelta > 0.08)
      ) {
        ...
        if (IS_SAFARI) {
          ...
          const css = `
            ...
            --js-card-header-bottom-alpha: ${(0.9 + (0 - 0.06 * cardFastness) * twoDVisualMix).toFixed(precision)};
          `;
          r.cssText = css;
        } else {
          ...
          r.setProperty(
            '--js-card-pixel-opacity',
            (0.94 + (-0.22 - 0.64 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-card-core-overlay-opacity',
            (1 + (-0.28 - 0.62 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-card-core-glow-opacity',
            (0.7 + (-0.28 - 0.37 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-card-shell-ring-alpha',
            (0.08 + (-0.02 - 0.022 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-card-shell-shadow-y',
            `${(20 + (-6 - 6 * cardFastness) * twoDVisualMix).toFixed(precision)}px`
          );
          r.setProperty(
            '--js-card-shell-shadow-blur',
            `${(50 + (-18 - 14 * cardFastness) * twoDVisualMix).toFixed(precision)}px`
          );
          r.setProperty(
            '--js-card-shell-shadow-alpha',
            (0.45 + (-0.17 - 0.12 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-two-d-card-outside-ring-alpha',
            (0.035 - 0.011 * cardFastness).toFixed(precision)
          );
          r.setProperty(
            '--js-two-d-card-outside-shadow-y',
            `${(8 - 4 * cardFastness).toFixed(precision)}px`
          );
          r.setProperty(
            '--js-two-d-card-outside-shadow-blur',
            `${(20 - 10 * cardFastness).toFixed(precision)}px`
          );
          r.setProperty(
            '--js-two-d-card-outside-shadow-alpha',
            (0.16 - 0.04 * cardFastness).toFixed(precision)
          );
          r.setProperty(
            '--js-card-header-top-alpha',
            (0.96 + (0 - 0.02 * cardFastness) * twoDVisualMix).toFixed(
              precision
            )
          );
          r.setProperty(
            '--js-card-header-bottom-alpha',
            (0.9 + (0 - 0.06 * cardFastness) * twoDVisualMix).toFixed(precision)
          );
        }
